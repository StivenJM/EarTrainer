...
const HighScoresScreen: React.FC<HighScoresScreenProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    ...
    // Remove duplicates based on sessionId or combination of fields
    const uniqueScores = scores.filter((score, index, self) => {
      if (score.sessionId) {
        return self.findIndex(s => s.sessionId === score.sessionId) === index;
      }
      // Fallback for old scores without sessionId
    ...
    localStorage.setItem('musicalEarHighScores', JSON.stringify(uniqueScores));
    
    setHighScores(uniqueScores);
  }, []);
      ...
      <div className="w-full max-w-4xl bg-white rounded-xl shadow-lg p-6 md:p-8">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center">
            ...
            <h1 className="text-3xl font-bold text-indigo-700">EarTrainer - Mejores Puntuaciones</h1>
          </div>
